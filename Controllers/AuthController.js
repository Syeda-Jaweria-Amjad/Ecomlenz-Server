...
const sortSellers = async (req, res) => {
  try {
    const { isSaved, pauseStatus } = req.query;
 
    if (!isSaved && !pauseStatus) {
      return res.status(400).json({
        success: false,
        message:
    ...
    // Fetch sellers with dynamic sorting
...
const SearchBySellerApi = async (req, res) => {
  try {
    ...
    if (sellers.length > 0) {
      // For each seller, find the count of products
      const result = await Promise.all(
        sellers.map(async (seller) => {
          ...
          return {
            ...
            date: seller.date,
            isSaved: seller.isSaved,
            pauseStatus: seller.pauseStatus,
            productCount,
          };
        })
      );
